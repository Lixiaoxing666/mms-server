...
module.exports = async (req, res) => {
  ...
  if (userList) {
    for (let item of userList) {
      ...
      const count = await Exrecord.find({
        export_user: user_id
      }).countDocuments()
      total += count
      const currentResult = await Exrecord.find({
        export_user: user_id
      }).populate('export_user')
  ...
  res.send({
    data: {
      exportList: queryResult,
      total
    },
    status: 200,
    message: '查询药品出库记录列表成功！'
  })
}